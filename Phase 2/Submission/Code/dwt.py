...
def displayDTWDistance(dir, filename, extension):
    distance = calculateDTWDistance(dir, filename + extension)
    n = 10
    topitems = heapq.nsmallest(n, distance.items(), key=itemgetter(1))  # Use .iteritems() on Py2
    topitemsasdict = dict(topitems)
    print("\nThe top 10 gestures similar to ", filename, " are: " + str([key[:-5] for key in topitemsasdict]))
