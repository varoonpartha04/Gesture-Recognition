...
def read_all_files(dir, output_directory):
    files = [f for f in listdir(output_directory) if isfile(join(output_directory, f)) and re.search('\.awrd', f)]
    data = {}
    for file in files:
...
def calculationDTW(list1, list2):
    total_distance = 0
        ...
        for j in range(1,n):
            for k in range(1,m):
                if list1[i][j] == list2[i][k]:
                    ...
                    dist[j][k] = abs(list1[i][j] - list2[i][k]) + min(dist[j-1][k], dist[j][k-1], dist[j-1][k-1])
        total_distance = total_distance + dist[n-1][m-1]
    return total_distance


def calculateDTWDistance(dir, filename):
        ...
        if 1 == 1:
            ...
            for rows in data[key]:
                ...
                if(prev[7] == vals[7]):
                    ...
                    smalllist.append(float(rows[1]))
            list2.append(smalllist)
            distance[key] = calculationDTW(list1,list2)

    return distance


def similarityMatrixDTW(dir):
    files = [f for f in listdir(dir) if isfile(join(dir, f)) and re.search('\.awrd', f)]
    ...
    for file1 in files:
        ...
        for file2 in files:
            if df[file1][file2] == 0:
                ...
                temp = calculationDTW(list1,list2)
                temp = temp / 100
...
def displayDTWDistance(dir, filename):
    distance = calculateDTWDistance(dir, filename)
    n = 10
...
# print(similarityMatrixDTW("/Users/varoon/MWDB/515ProjectFall2020/Phase 2/Code/Outputs"))
# displayDTWDistance('/Users/zainmomin/Desktop/phase2F/515ProjectFall2020/Phase 2/Code/Outputs', '5.awrd')
